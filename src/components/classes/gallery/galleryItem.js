...
export class GalleryItem {
  constructor(el, ingalleryPosition, animatableProperties, images) {
    this.images = images
    // initialize Splitting

    // el is the <a> with class "gallery__item"
    this.DOM = { el: el };
    // position in the gallery
    this.ingalleryPosition = ingalleryPosition;
  ...
  // we want to add/append to the gallery item the following html:
  ...
  layout() {
    ...
    this.DOM.revealImage.style.backgroundImage = `url(${this.images[this.ingalleryPosition][1]})`;
    ...
    // text element
    this.DOM.textInner = this.DOM.el.querySelector('.gallery__item-text');
  ...
  // calculate the position/size of both the gallery item and reveal element
  calcBounds() {
    this.bounds = {
  ...
  render() {
    ...
    this.animatableProperties.rotation.current = this.firstRAFCycle ? startingAngle : map(mouseDistanceX, 0, 300, startingAngle, cursorDirection.x < 0 ? startingAngle + 100 : startingAngle - 100);
    ...
    this.animatableProperties.tx.previous = this.firstRAFCycle
      ...
      : lerp(this.animatableProperties.tx.previous, this.animatableProperties.tx.current, this.animatableProperties.tx.amt);
    this.animatableProperties.ty.previous = this.firstRAFCycle
      ? this.animatableProperties.ty.current
      : lerp(this.animatableProperties.ty.previous, this.animatableProperties.ty.current, this.animatableProperties.ty.amt);
    this.animatableProperties.rotation.previous = this.firstRAFCycle
      ? this.animatableProperties.rotation.current
      : lerp(this.animatableProperties.rotation.previous, this.animatableProperties.rotation.current, this.animatableProperties.rotation.amt);
    this.animatableProperties.brightness.previous = this.firstRAFCycle
      ? this.animatableProperties.brightness.current
      : lerp(this.animatableProperties.brightness.previous, this.animatableProperties.brightness.current, this.animatableProperties.brightness.amt);
